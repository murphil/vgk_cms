...
// 需要在 mailAccount.yml 中填写正确信息，然后执行 npm run x -- y . 生成 mailAccount.yml.json 文件
let smtpConfig = require('../../account/mailAccount.yml.json')
...
body,pre{ background:#fdf6e3 top left;}
body{
  color:#657b83;
  background-color: #fdf6e3;
}
...
transporter.use('compile', function(mail, callback){
  if(mail.data.html && !mail.data.noStyle){
    mail.data.html = stylesheet + `<div class="markdown-body">${mail.data.html}</div>`}
  callback();
});
...
module.exports = {
  sendMail : {
    ...
    resolve: async (i, logger, cmd) => {
      i.mail.from = smtpConfig.auth.user
      i.mail.to = i.target
      let res = await transporter.sendMail(i.mail)
